...
class YamlConfigDataLoader(FileConfigDataLoader):
    def __init__(
            self,
            file_name: str,
        ...
        try:
            ...
            raise RuntimeError(f"Module yaml required for YamlConfigDataLoader. "
                               f"Use pip install toml or uv / poetry add toml as appropriate.")
        self.yaml = yaml

    def _read_file(self, file_path: Path) -> MutableMapping:
        self.log.info(f"Reading {file_path}")
        self.files_read.append(file_path)
        with file_path.open('rt', encoding='utf8') as yaml_content:
            config_data = self.yaml.safe_load(yaml_content)
        return config_data

    def save_config_data(self, config_data: ConfigRoot):
        file_path = Path(self.start_path, self.file_name)
        config_data_yaml_ready = YamlConfigDataLoader.prepare_config_data_for_save(config_data)
        with file_path.open('wt', encoding='utf8') as config_file:
            self.yaml.dump(config_data_yaml_ready, config_file)
        ...
        if isinstance(field_value, bool):
            ...
            for key, value in field_value.items():
                if not isinstance(key, str):
                    str_keys = False
                field_value[key] = YamlConfigDataLoader.format_value_for_save(value)
            if not str_keys:
                field_value = str(field_value)
            else:
                pass
    ...
    @staticmethod
    def prepare_config_data_for_save(config: ConfigRoot, default_delimiter='\n', parents=None) -> dict:
        if parents is None:
            parents = []
        ...
        for field_name, field_info in config.model_fields.items():
            ...
            if lenient_issubclass(field_info.annotation, BaseModel):
                config_data_dict[field_name] = YamlConfigDataLoader.prepare_config_data_for_save(
                ...
                for sub_section_number, sub_section_value in enumerate(getattr(config, field_name)):
                    ...
                    config_data_dict[sub_section_id] = YamlConfigDataLoader.prepare_config_data_for_save(
                        sub_section_value,
                    )
                config_data_dict[field_name] = section_name_list
            elif lenient_issubclass(field_info.annotation, List):
                value_list = [YamlConfigDataLoader.format_value_for_save(v) for v in field_value]
                ...
                # Use python format
                config_data_dict[field_name] = YamlConfigDataLoader.format_value_for_save(field_value)
